<?php 
$currentPage = 'about'; 
include ('header.php');

?> 

<div id="container">
	<div class="mission">
        <h1>- OUR MISSION -</h1>
        <p>
            At Apptopia, we all come to work every day because we want to solve the biggest problem in mobile. Everyone is guessing. Publishers don’t know what apps to build, how to monetize them, or even what to price them at. Advertisers &amp; brands don’t know where their target users are, how to reach them, or even how much they need to spend in order to do so. Investors aren’t sure which apps and genres are growing the quickest, and where users are really spending their time (and money).
            <br>
            <br>
            Throughout the history of business, people use data to make more informed decisions. Our mission at Apptopia is to make the app economy more transparent. Today we provide the most actionable mobile app data &amp; insights in the industry. We want to make this data available to as many people as possible (not just the top 5%).
        </p>
    </div>
     <div class="about-gray-line">
    </div>
    <div class="about">
        <h1>- ABOUT -</h1>
        <img src="images/facebook-default.png" alt="" id="facebook">
        <h3>Drilon Arifi</h3>
        <p>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloremque inventore nisi odio similique quam molestiae vitae, debitis aut deleniti, placeat asperiores facilis hic. Nihil molestiae sit, dolor ea similique optio.Lorem ipsum dolor sit amet, consectetur adipisicing elit. Dolorum hic recusandae culpa in rerum labore incidunt delectus officia! In, quos nihil cupiditate. Earum quod natus unde quas, consequatur libero eum.Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aliquam, illum blanditiis distinctio maiores saepe totam suscipit alias cupiditate similique? Fugiat corporis enim debitis, reiciendis aliquam neque vel. Itaque, fugiat qui?
        </p>
    </div>
</div>	
<?php
 
	include('footer.php');
?>
